...
struct webRequest
{
    char uri[BUFFER]; char method[BUFFER]; char version[BUFFER]; char *body;
...
int sendHeader(char* key, char* value, int new_fd){
    char resp[BUFFER] = "HTTP/1.0 200 OK\r\n"
                  "Server: webserver-c\r\n";
    if(send(new_fd, resp, strlen(resp), 0)== -1){perror("header sending");return 1;}
...
int sendFileWeb(char *filename, char *contentType, int new_fd){
    ...
    char ch[sz];
    rewind(fp);
    if(fread(ch, 1,sz,  (FILE *)fp) < 0){perror("reading");return 1;}
    if(sendHeader("Content-Type", contentType, new_fd) != 0){return 1;}
    if(send(new_fd, ch, sz, 0)==-1){perror("sending file"); return 1;}
    return 0;
}
